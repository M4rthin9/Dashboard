<script lang="ts">
  ...
  const variants: Record<string, string> = {
    ...
    secondary:
      'bg-slate-800 text-white hover:bg-slate-900 focus-visible:outline-slate-800 disabled:hover:bg-slate-800 dark:bg-slate-200 dark:text-slate-900 dark:hover:bg-white',
    outline:
...
<button
  ...
  {onclick}
  {disabled}
  class="inline-flex items-center justify-center gap-2 rounded-lg font-medium transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 disabled:cursor-not-allowed disabled:opacity-60 {variants[variant]} {sizes[size]} {fullWidth ? 'w-full' : ''}"
>
  {#if loading}
