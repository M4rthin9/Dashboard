...
<Card title="แผนผังโต๊ะ" subtitle="เฉพาะรายการสถานะ เสร็จสิ้น / รอชำระเงิน">
  <div class="mb-3 flex items-center gap-2">
    <label for="floor-plan-date" class="text-sm font-medium text-slate-600 dark:text-slate-300">เลือกวันที่</label>
    <select
      id="floor-plan-date"
      bind:value={selectedDate}
      class="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm focus:border-indigo-500 focus:outline-none dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
    >
      {#each dates as d (d)}
      ...
      {#each dayBookings as r, i (r.ref)}
        <div class="rounded-xl border p-3 {normalizeStatus(r.status) === 'รอชำระเงิน' ? 'border-amber-300 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/40' : 'border-green-300 bg-green-50 dark:border-green-900 dark:bg-green-950/40'}">
          <div class="mb-1 flex items-center justify-between">
            ...
            <span class="rounded-full px-2 py-0.5 text-[10px] font-medium {normalizeStatus(r.status) === 'รอชำระเงิน' ? 'bg-amber-500/20 text-amber-700 dark:text-amber-300' : 'bg-green-500/20 text-green-700 dark:text-green-300'}">
          ...
          <div class="text-xs font-semibold" style="color:{wingColors[String(r.wing ?? '')] ?? '#475569'}">{r.wing ?? '—'}</div>
