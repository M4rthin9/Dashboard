...
<div class="flex flex-col gap-4">
  <Card title="บันทึกเหตุการณ์" subtitle="ประวัติการใช้งานระบบ">
          ...
          <table class="w-full min-w-[760px] text-left text-sm">
            <thead>
              <tr class="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-500 dark:border-slate-700 dark:text-slate-400">
                ...
                <th class="px-3 py-2 font-medium">ผู้ใช้</th>
                <th class="px-3 py-2 font-medium">การกระทำ</th>
                <th class="px-3 py-2 font-medium">REF</th>
                <th class="px-3 py-2 font-medium">รายละเอียด</th>
                <th class="px-3 py-2 font-medium">ผลลัพธ์</th>
            ...
            <tbody>
              {#each paged as log (log.timestamp + log.username + log.action)}
                <tr class="border-b border-slate-100 last:border-0 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                  <td class="px-3 py-2.5 whitespace-nowrap text-xs text-slate-500 dark:text-slate-400">
                    {formatDateTimeThai(log.timestamp)}
                  </td>
                  <td class="px-3 py-2.5 font-medium">{log.username}</td>
                  ...
                  <td class="max-w-[220px] truncate px-3 py-2.5 text-xs text-slate-600 dark:text-slate-300">{log.details ?? ''}</td>
                  <td class="px-3 py-2.5">
                    <span class="rounded-full px-2 py-0.5 text-[11px] font-medium {log.result === 'success' ? 'bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-300' : log.result === 'denied' ? 'bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300' : log.result === 'error' ? 'bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'}">
                      {log.result ?? ''}
                    </span>
                  </td>
